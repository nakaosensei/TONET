...
class TonetDataSet():
    ...
    def preProcessDataset(self):
        ...
        return filteredData
    '''
    def loadPandasDataset(self):      
        #dfOriginal = pd.DataFrame(database)
        dfOriginal = pd.read_json('../data/TONet/'+'upscaledDatabase.json')
           
        #dfOriginal.to_csv('../data/TONet/'+'upscaledDatabase.csv',index=False)
        self.labelsHashMap = {}
        self.labelsStr = dfOriginal[labelsColumn]   
        for lb in self.labelsStr:            
            if lb not in self.labelsHashMap:
                self.labelsHashMap[lb]=len(self.labelsHashMap.keys())  
        
        dfReduzido=dfOriginal.drop(settingsJson['colunasNaoConsideradas'],axis = 1)        
        self.tensorDatabase = df_to_tensor(dfReduzido)  
    '''
